    ...
    <div style={{ width: "100%", maxWidth: 480, margin: "0 auto" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
        <input
          ...
          style={{ padding: "14px 18px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#fff", fontSize: 15, outline: "none" }}
        />
        <input
          value={phone} onChange={e => setPhone(e.target.value)}
          placeholder="Ваш телефон (+998...)"
          style={{ padding: "14px 18px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#fff", fontSize: 15, outline: "none" }}
        />
        <select
          value={type} onChange={e => setType(e.target.value)}
          style={{ padding: "14px 18px", background: "#0D1B2A", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#fff", fontSize: 15, outline: "none" }}
        >
      ...
      <button onClick={send} disabled={loading} style={{
        width: "100%", padding: "18px", background: "#C9A84C", color: "#080C12",
...
export default function Home() {
  ...
  const plans = [
    {
      name: "Базовый", price: "625 000", usd: "$50", desc: "Веб-плеер для старта",
      features: ["Доступ через браузер", "1 жанр на выбор", "Официальный сертификат", "Базовая поддержка"],
      highlight: false,
    },
    {
      name: "Стандарт", price: "875 000", usd: "$70", desc: "Android TV + автоматизация",
      features: ["Автозапуск при включении", "Все 27 жанров", "Официальный сертификат", "Приоритетная поддержка"],
      highlight: true,
    },
    {
      name: "Премиум", price: "1 250 000", usd: "$100", desc: "Для сетей от 5 точек",
  ...
  return (
    <main style={{ fontFamily: "'Georgia', serif", background: "#080C12", color: "#E8EFF5", overflowX: "hidden" }}>
      ...
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "120px 48px 80px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "10%", right: "-5%", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(26,107,154,0.15) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: "0%", left: "30%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(201,168,76,0.08) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ maxWidth: 1200, margin: "0 auto", width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 16px", background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 100, fontSize: 12, color: "#C9A84C", letterSpacing: "0.1em", marginBottom: 32, fontFamily: "monospace" }}>
              ♪ ЛИЦЕНЗИОННАЯ ФОНОВАЯ МУЗЫКА
            </div>
            <h1 style={{ fontSize: 64, fontWeight: 400, lineHeight: 1.1, marginBottom: 24, color: "#fff", fontStyle: "italic" }}>
              Музыка для<br />
            ...
            <p style={{ fontSize: 18, color: "#8BA7BE", lineHeight: 1.7, marginBottom: 48, maxWidth: 480 }}>
              Легальная фоновая музыка для ресторанов, магазинов и кафе в Узбекистане. Официальный сертификат. Автоматически. Без забот.
            </p>
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              <a href="#contact" style={{ padding: "16px 36px", background: "#C9A84C", color: "#080C12", borderRadius: 8, fontSize: 16, fontWeight: 700, textDecoration: "none" }}>
                Первые 10 дней бесплатно →
              </a>
              <a href="#how" style={{ padding: "16px 24px", color: "#8BA7BE", fontSize: 14, textDecoration: "none" }}>Как это работает ↓</a>
            </div>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ background: "linear-gradient(135deg, #0D1B2A, #162435)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 24, padding: 40, boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
              <div style={{ fontSize: 12, color: "#C9A84C", letterSpacing: "0.1em", marginBottom: 24, fontFamily: "monospace" }}>▶ СЕЙЧАС ИГРАЕТ</div>
              <div style={{ fontSize: 48, fontWeight: 700, color: "#fff", marginBottom: 8, transition: "all 0.5s ease", fontStyle: "italic" }}>{genres[activeGenre]}</div>
              <div style={{ fontSize: 14, color: "#8BA7BE", marginBottom: 32 }}>Автоматически подобранный плейлист</div>
              <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 48, marginBottom: 32 }}>
                {[...Array(20)].map((_, i) => (
                  <div key={i} style={{ flex: 1, background: i % 3 === 0 ? "#C9A84C" : "#1A6B9A", borderRadius: 2, opacity: 0.7, height: `${20 + Math.sin(i * 0.8 + activeGenre) * 60}%`, transition: "height 0.5s ease", animation: `wave ${0.5 + i * 0.1}s ease-in-out infinite alternate` }} />
                ))}
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["Ресторан", "Кафе", "Магазин", "Отель", "Салон", "Фитнес"].map(tag => (
                  <span key={tag} style={{ padding: "4px 12px", background: "rgba(26,107,154,0.2)", border: "1px solid rgba(26,107,154,0.3)", borderRadius: 100, fontSize: 12, color: "#8BA7BE" }}>{tag}</span>
                ))}
              </div>
            </div>
            <div style={{ position: "absolute", top: -20, right: -20, background: "#C9A84C", color: "#080C12", padding: "12px 20px", borderRadius: 12, fontSize: 13, fontWeight: 700, boxShadow: "0 8px 24px rgba(201,168,76,0.4)" }}>
      ...
      <section style={{ padding: "100px 48px", background: "#0A0F18" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div style={{ fontSize: 12, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 16, fontFamily: "monospace" }}>ПРОБЛЕМА</div>
            <h2 style={{ fontSize: 44, fontWeight: 700, color: "#fff" }}>Незаконная музыка —<br />угроза для бизнеса</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 24 }}>
            {[
              ...
              <div key={item.title} style={{ padding: 32, background: "#0D1B2A", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16 }}>
      ...
      <section id="how" style={{ padding: "100px 48px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div style={{ fontSize: 12, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 16, fontFamily: "monospace" }}>КАК ЭТО РАБОТАЕТ</div>
            <h2 style={{ fontSize: 44, fontWeight: 700, color: "#fff" }}>Просто. Быстро. Легально.</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 24 }}>
            {steps.map((step, i) => (
              <div key={step.num} style={{ padding: 32, background: "#0D1B2A", border: `1px solid ${i === 0 ? "rgba(201,168,76,0.4)" : "rgba(255,255,255,0.06)"}`, borderRadius: 16, position: "relative" }}>
                <div style={{ fontSize: 48, fontWeight: 700, color: i === 0 ? "#C9A84C" : "rgba(26,107,154,0.5)", marginBottom: 16 }}>{step.num}</div>
                ...
                {i < 3 && <div style={{ position: "absolute", right: -16, top: "50%", transform: "translateY(-50%)", color: "#C9A84C", fontSize: 20, zIndex: 1 }}>→</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="pricing" style={{ padding: "100px 48px", background: "#0A0F18" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div style={{ fontSize: 12, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 16, fontFamily: "monospace" }}>ТАРИФЫ</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 24 }}>
            {plans.map(plan => (
              <div key={plan.name} style={{ padding: 40, background: plan.highlight ? "linear-gradient(135deg, #1A6B9A, #0D3D5E)" : "#0D1B2A", border: `2px solid ${plan.highlight ? "#C9A84C" : "rgba(255,255,255,0.06)"}`, borderRadius: 20, position: "relative", transform: plan.highlight ? "scale(1.05)" : "scale(1)", boxShadow: plan.highlight ? "0 20px 60px rgba(26,107,154,0.3)" : "none" }}>
                {plan.highlight && <div style={{ position: "absolute", top: -16, left: "50%", transform: "translateX(-50%)", background: "#C9A84C", color: "#080C12", padding: "6px 20px", borderRadius: 100, fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", whiteSpace: "nowrap" }}>РЕКОМЕНДУЕМ</div>}
                ...
                <div style={{ marginBottom: 8 }}>
                  <span style={{ fontSize: 32, fontWeight: 700, color: plan.highlight ? "#fff" : "#C9A84C" }}>{plan.price}</span>
                ...
                <a href="#contact" style={{ display: "block", textAlign: "center", padding: "14px", borderRadius: 8, background: plan.highlight ? "#C9A84C" : "rgba(201,168,76,0.1)", border: plan.highlight ? "none" : "1px solid rgba(201,168,76,0.3)", color: plan.highlight ? "#080C12" : "#C9A84C", fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
                  Подключить →
                </a>
              </div>
            ))}
          ...
          <p style={{ textAlign: "center", fontSize: 13, color: "#8BA7BE", marginTop: 40 }}>
            ★ Первые 10 дней — бесплатно для новых клиентов
          </p>
        </div>
      </section>
      ...
      <section id="contact" style={{ padding: "100px 48px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <div style={{ fontSize: 12, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 24, fontFamily: "monospace" }}>НАЧНИТЕ СЕГОДНЯ</div>
      ...
      <footer style={{ padding: "32px 48px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, color: "#8BA7BE" }}>
